...
import { FileText, RefreshCcw, Settings, Upload ,CornerUpLeft, Trash, Loader} from "lucide-react";
...
const sidebarItems = [
  { name: "Assistant", icon: FileText },
  { name: "Research", icon: FileText },
];

// Array for document items

export default function Component() {
  // const [data, setData] = useState<string>("");
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  // const [pdfData, setPdfData] = useState<string>("");
  const [selectedNamespace, setSelectedNamespace] = useState<string | null>(null); // State for selected namespace
  const [loading, setLoading] = useState<boolean>(false); // Loading state

  const [prompt, setPrompt] = useState<string>("");

  const [documentItems, setDocumentItems] = useState<{ name: string; url: string }[]>([]); // State for document items
...
formData.forEach((value, key) => {
  ...
  const handleClick = async () => {
    try {
      ...
      const response = await axios.post('/api/user/chat', { query: prompt, namespace: selectedNamespace });
  ...
  const handleDeleteDocument = async (name: string) => {
    try {
      ...
      // const response = await axios.delete(`/api/user/pdf/delete?name=${encodedName}`); // Change to DELETE method
  
      // if (response.status === 200) { // Check for successful response
        setDocumentItems((prevItems) => prevItems.filter((doc) => doc.name !== name));
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <Sidebar className="w-[10%] bg-white border-r">
        <div className="p-4">
          <h1 className="text-xl font-bold">Harvey.</h1>
        </div>
          ...
          <footer className="border-t p-4">
            ...
            <div className="mt-4 ">
              <div className="flex text-xs font-semibold  justify-between"> 
                ...
                <div className="flex">
                  <Button className="p-3 mb-2 " onClick={fetchDocumentItems} size={'sm'}>
      ...
      />
            ...
            <Button className="mb-2  p-3  " size='sm' onClick={handleFileUpload}>Upload</Button>
                  ...
                  >
                    ...
                    <div className="flex">
                      <Button variant="ghost" size="sm" onClick={() => handleViewDocument(doc.name)}>
                        <CornerUpLeft className={`h-3 w-3 ${selectedNamespace === doc.name ? 'bg-black text-white' : 'bg-gradient-to-r from-gray-100 to-gray-200'}`} />
